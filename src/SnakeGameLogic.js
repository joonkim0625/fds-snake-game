...
SnakeGameLogic.prototype.nextState = function () {
  ...
  if (this.direction === 'up') {
    this.joints.pop()
    this.joints.unshift({
      x: this.joints[0]. x, y: this.joints[0].y - 1  })
      ...
      this.joints.pop()
      this.joints.unshift({
        x: this.joints[0].x - 1, y: this.joints[0].y
      }) 
    } else {
      this.joints.pop()
      this.joints.unshift({
    ...
    if (this.joints[0].x === this.fruit.x && this.joints[0].y === this.fruit.y) {
      ...
      if (this.direction === 'up') {
        ...
        this.joints.push({
          x: this.joints[this.joints.length - 1].x, y: this.joints[this.joints.length - 1].y - 1
        })
      } else if (this.direction === 'left') {
        this.joints.push({
          x: this.joints[this.joints.length - 1].x + 1, y: this.joints[this.joints.length - 1].y 
        })
      } else {
        this.joints.push({
          x: this.joints[this.joints.length - 1].x - 1, y: this.joints[this.joints.length - 1].y
    ...
    for (let i = 1; i < this.joints.length; i++) {
      if (this.joints[i].x === this.fruit.x && this.joints[i].y === this.fruit.y) {
        this.fruit = {
          x: Math.floor(Math.random() * COLS),
          y: Math.floor(Math.random() * ROWS)
        }
    ...
    // 벽에 닿으면 게임이 끝남
    if (this.joints[0].x < 0 || this.joints[0].y < 0 || this.joints[0].x >= COLS || this.joints[0].y >= ROWS) {
      return false
    }
